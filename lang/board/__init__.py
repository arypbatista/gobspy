...
class Cell(object):
    ...
    def take(self, coli):
        ...
        Color. Raise a SelfDestructionException if
        there are no stones of the given color."""
        cnt = self.stones.get(coli, 0)
        if cnt > 0:
            self.stones[coli] = cnt - 1
        else:
            raise SelfDestructionException(t('Cannot take stones'))
    ...
    def init_color_properties(cls):
        ...
        for colname, coli in COLORS_BY_NAME.items():
...
class Board(object):
    ...
    def move(self, direction):
        ...
        if 0 <= y and y < height and 0 <= x and x < width:
            ...
            raise SelfDestructionException(t('Cannot move'))
    ...
    def pop_state(self):
        ...
        for change in seq_reversed(changelog_to_undo):
