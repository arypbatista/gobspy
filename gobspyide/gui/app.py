...
from ..common import tools, i18n, utils, LicenseFile
from ..common.utils import SourceException
from . import editor, viewer, config
...
def build_menu(menu, root, toplevel):
  ...
  while menu != []:
    ...
    if isinstance(submenu, list):
      m.add_cascade(label=name, menu=build_menu(submenu, m, toplevel),
                    underline=underline)
      continue
...
def menu_font_family(gui):
  ...
  menu = []
  submenu = []
  fams = filter(lambda x: 'a' <= x[0].lower() and x[0].lower() <= 'z', tkFont.families())
...
def board_to_string(board):
  return tools.formats.AvailableFormats['gbb']().to_string(board, style='compact')

def board_from_string(board, s):
  return tools.formats.AvailableFormats['gbb']().from_string(s, board)
...
class InterpreterWorker(object):
  def run(self, queue_in, queue_out):
    ...
    board = tools.Board((1, 1))
    board_from_string(board, board_repr)
    try:
      queue_out.put(('LOG', i18n.i18n('Starting program execution.')))
      res = tools.run(current_text, filename, board)
      if res.failed():
        queue_out.put(('FAIL', (SourceException, (res.error, None))))
      else:
        queue_out.put(('OK', board_to_string(res.final_board), res.result))
    except SourceException as exception:
      # Note: we cannot send an exception directly through a multiprocessing.Queue
      # (it has to do with the requirement of the object being serializable)
      queue_out.put(('FAIL', (exception.__class__, (exception.message, exception.area))))
...
class GUI(Tkinter.Tk):
  ...
  def __init__(self, *args, **kwargs):
    ...
    self._font = config.DefaultFont
    self.tools = tools

    eframe = Tkinter.Frame(self)
    self.editor = editor.Editor(self.tools, eframe, font=self._font)
  ...
  def _init_filetypes(self):
    self.filetypes = {
      'gbs': [
        (i18n.i18n('Gobspy source'), '*.gpy', 'TEXT'),
  ...
  def file_open(self, *args):
    fn = tkFileDialog.askopenfilename(parent=self,
  ...
  def file_save_as(self, *args):
    ...
    fn = tkFileDialog.asksaveasfilename(parent=self,
            title=i18n.i18n('Save file'),
            filetypes=self.filetypes['gbs'],
            initialdir=self._last_directory)
    self.editor.enable_return()
    if fn == None or fn == (): return False
    fn = fn.strip(' ')
    if fn == '': return False
    if '.' not in fn:
      fn += '.gbs'
  ...
  def _filename_or_untitled(self):
    if self.editor.has_filename():
      return os.path.basename(self.editor.filename())
    else:
      return i18n.i18n('Untitled')

  def _parse_contents(self):
    return self.tools.parse_string(self.editor.current_text(),
                                   self._filename_or_untitled())

  def _prepare_for_next_run(self):
    self._run = CurrentRun()
    self._run.running = False
    self._run.queue_out = multiprocessing.Queue()
    self._run.queue_in = multiprocessing.Queue()
    self._run.worker = InterpreterWorker()
    # worker's queue_in is app's queue_out and vice-versa:
    self._run.process = multiprocessing.Process(target=self._run.worker.run, args=(self._run.queue_out, self._run.queue_in,))
  ...
  def gobstones_start_run(self, *args):
    ...
    self._last_alarm_id = self.after(config.TickDelay, self.gobstones_continue_run)
  ...
  def gobstones_end_run(self, result):
    ...
    if len(result) > 0:
      result_msg = map(lambda var, val: '%s -> %s\n' % (var, val), result)
  ...
  def gobstones_fail_run(self, exception):
    ...
    self.editor.show_error(exception)
    if self.viewer:
      if isinstance(exception, utils.DynamicException):
  ...
  def open_viewer(self):
    if not self.viewer:
      self.viewer = viewer.BoardViewer(self.tools, parent_gui=self, font=self._font)
  ...
  def board_change_size(self, *args):
    ...
    nw = tkSimpleDialog.askstring(
           i18n.i18n('Change board size'),
           i18n.i18n('Width:'),
           initialvalue='%s' % (w,))
    nh = tkSimpleDialog.askstring(
           i18n.i18n('Change board size'),
           i18n.i18n('Height:'),
           initialvalue='%s' % (h,))

    if not utils.is_int(nw) or not utils.is_int(nh):
  ...
  def board_load_fn(self, fn):
    ...
    if not self.viewer:
      self.open_viewer()
    fmt = tools.formats.format_for(fn)
