...
from . import progress, config
from ..common.tools import StringIO
...
def tposx(row0, pos):
    return '%s.%s' % (row0 - 1 + pos[0], pos[1] - 1)
def tpos(row0, pos):
...
- status bar with filename and line numbers"""
  ...
  def __init__(self, tools, root, expandTabs=True, tabStop=4, autoindent=False, font=config.DefaultFont, *args, **kwargs):
  ...
  def indent_region(self, *args):
    try:
      ...
      return
    ts = self.tabStop
    row0, col0 = map(int, self.index('sel.first').split('.'))
    row1, col1 = map(int, self.index('sel.last').split('.'))
    if col1 == 0: row1 -= 1
  ...
  def sync(self, row_from, row_to, clean=True, indent=False):
    ...
    class Token:
        def __init__(self, type, token, start, end):
            class Pos:
            ...
            self.start = start
            self.end = end
            self.pos_begin = Pos(start)
            self.pos_end = Pos(end)


    def token_eater(token_type, token_value, start, end, string):
        token = Token(token_type, token_value, start, end)
        s = token.pos_begin.col
        e = token.pos_end.col
        tag = token.type
        ...
        sindex = tpos(row_from, token.pos_begin)
        eindex = tpos(row_from, token.pos_end)
        self.tag_add(tag, sindex, eindex)

        # indentation after a newline
        if res.indent and index_gt(eindex, cursor_index):
          if len(opening) == 0:
            res.indent_col = 0
          elif opening[-1].value == '(':
            res.indent_col = opening[-1].pos_begin.col
          elif opening[-1].value == '{':
            res.indent_col = self.tabStop * len(opening)
          res.indent = False
        ...
        if token.value in '(){}':
          ...
          elif token.value in ')}' and len(opening) > 0:
             op = opening.pop()
             if closing_delimiter(op.value) == token.value:
    ...
    try:
        self.tools.tokenize(StringIO(string).readline, token_eater)
    except Exception as e:
        if "EOF in multi-line statement" == e.args[0]:
            # User has written a lonely bracket
            pass
        else:
            raise e

    # after
    return res.indent_col
  ...
  def start_run(self, onstop):
    self.end_run()
    self.progress_bar = progress.ProgressBar(self.statusbar, onstop=onstop)
    self.progress_bar.place(x=0, y=0, relwidth=1, relheight=1)

  def end_run(self):
    if self.progress_bar == None: return
