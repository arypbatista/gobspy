...
class BoardFrame(Tkinter.Canvas):
  ...
  def __init__(self, tools_, board, readonly, font, h_scroll, v_scroll, *args, **kwargs):
  ...
  def keypress(self, event):
    ...
    if self.mark:
      ...
      y, x = self.board.head
      col = None
    if event.char in '0123456789' and col != None:
      n = int(event.char)
      ...
      if col != None:
        self.add_mark(x, y, col)
    elif event.char == '+' and col != None:
      self.mouse_button1(event)
    elif event.char == '-' and col != None:
      self.mouse_button3(event)
    elif event.char in 'hH' and col != None:
      self.board.head = (y, x)
      self.refresh_aspect()
      self.add_mark(x, y, col)
      self.submit_change()
  ...
  def draw_stone(self, x0, y0, x1, y1, col, count):
    sm = (x1 - x0) / 8
    fill = i18n(Colors['stone_fill%i' % (col,)])
    outline = i18n(Colors['stone_border%i' % (col,)])
...
class BoardViewer(Tkinter.Tk):
  ...
  def __init__(self, tools_, parent_gui=None, font=config.DefaultFont, *args, **kwargs):
  ...
  def _fn_title(self, fn):
    self.title(i18n('Board viewer') + ' - ' + fn)
  ...
  def _build_scrolled_board_frame(self):
    ...
    self.board1_frame = BoardFrame(self.tools, self.board1, True, self._font,
                                   self.h_scroll, self.v_scroll,
                                   self.board_frame)
    self.board0_frame = BoardFrame(self.tools, self.board0, False,  self._font,
                                   self.h_scroll, self.v_scroll,
                                   self.board_frame,
  ...
  def save_board(self, fn, num, fmt='gbb'):
    ...
    try:
      if num == 0:
        ...
        raise Exception(i18n('There is no board to save.'))
    except Exception as e:
      self.show_error(e)
      f.close()
      return
  ...
  def prepare_for_run(self):
    if self.filename == None and not self.board0_frame.has_changed:
      self.randomize()
    else:
      self.reset_board1()
...
class Boom(Tkinter.Canvas):
  ...
  def draw_zig_zag_box(self, bbox, color, var):
    ...
    verts = map(lambda x: (int(x[0] + rdx()), int(x[1] + rdy())), verts)
